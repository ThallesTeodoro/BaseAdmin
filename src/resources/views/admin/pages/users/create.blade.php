...
@section('title', config('adminlte.title') . ' - Adicionar Novo')
    ...
    <form action="{{ route('admin.users.store') }}" method="POST">
        ...
        <div class="box-body">
            <div class="form-group {{ ($errors->has('name')) ? 'has-error' : '' }}">
              ...
              <input type="text" class="{{ $errors->has('name') ? 'form-control is-invalid' : 'form-control' }}" id="name" name="name" value="{{ old('name' )}}">
              @if($errors->has('name'))
            ...
            <div class="form-group {{ ($errors->has('email')) ? 'has-error' : '' }}">
              ...
              <input type="email" class="{{ $errors->has('email') ? 'form-control is-invalid' : 'form-control' }}" id="email" name="email" value="{{ old('email' )}}">
              @if($errors->has('email'))
                <div class="invalid-feedback">
                    {{ $errors->first('email') }}
                </div>
            ...
            <div class="form-group {{ ($errors->has('user_role')) ? 'has-error' : '' }}">
                ...
                <select name="user_role" id="user_role" class="{{ $errors->has('user_role') ? 'form-control is-invalid' : 'form-control' }}">
                    ...
                    <option value="{{ $key }}" {{ old('user_role') == $key ? 'selected' : '' }}>
                        {{ $role }}
                    </option>
                    @endforeach
                </select>
